...
<head>
	...
	<style>
		...
		h1 { color: #333; }
		.album-item { margin-bottom: 20px; padding: 10px; background: #fafafa; border: 1px solid #ddd; }
		.album-title { font-weight: bold; font-size: 1.2em; color: #007BFF; }
	</style>
</head>
<body>

	<h1>Задание 14: Вывод альбомов и треков</h1>
	<hr>
	...
	$tracks = array(
		...
		// ... (остальные треки из вашего списка корректно учтены)
	...
	// Проверяем, передан ли GET-параметр id_album
	if (isset($_GET['id_album'])) {
		$search_id = $_GET['id_album'];
		$found = false;

		// Вариант: искомый альбом и его треки
		foreach ($albums as $album) {
			if ($album['id_album'] == $search_id) {
				$found = true;
				echo "<div class='album-item'>";
				echo "<div class='album-title'>Альбом: {$album['title']} ({$album['date']})</div>";
				echo "<ul>";
				
				// Вложенный цикл поиска связанных треков (первичный ключ id_album -> внешний ключ id_album)
		...
		if (!$found) {
			echo "<p style='color: red;'>Альбом с id = " . htmlspecialchars($search_id) . " не найден.</p>";
			echo "<p><a href='?'>Показать все альбомы</a></p>";
		}

	} else {
		// Вариант: полный список всех альбомов и их треков в виде вложенных списков
		echo "<p>Для фильтрации добавьте в адресную строку параметр, например: <code>?id_album=4</code></p>";
		
		foreach ($albums as $album) {
			echo "<div class='album-item'>";
			echo "<div class='album-title'>Альбом: <a href='?id_album={$album['id_album']}'>{$album['title']}</a> ({$album['date']})</div>";
			echo "<ul>";
			
			// Вложенный цикл для поиска треков текущего альбома
			foreach ($tracks as $track) {
				if ($track['id_album'] == $album['id_album']) {
					echo "<li>{$track['name']}</li>";
				}
