...
<head>
	<meta charset="UTF-8">
	<title>Задание 3 — Поиск треков</title>
	<style>
		/* Оформление таблицы строго по макету из задания */
		table { border-collapse: collapse; width: 80%; margin-top: 20px; font-family: Arial, sans-serif; }
...
<body>
	...
	<h1>Вывод треков по идентификатору альбома</h1>
	<hr>

	<?php
	// === 1. Включение данных (массив $track) ===
	$track = array(
	...
	function fnOutTrack($arr, $album_id) {
		// Формируем структуру таблицы согласно макету
		echo "<table>";
		echo "<thead>
				<tr>
					<th>ID трека</th>
		...
		// Фильтруем массив и выводим только те строки, у которых id_album совпадает с искомым
		foreach ($arr as $item) {
			if ($item['id_album'] == $album_id) {
				echo "<tr>";
	...
	// === 3. Вызов созданной функции ===
	$id = 10; // Идентификатор альбома для поиска
	
	echo "<h3>Отображение треков альбома с ID = " . $id . ":</h3>";
	
	// Передаем в функцию весь массив треков и нужный нам ID
	fnOutTrack($track, $id);
	?>
